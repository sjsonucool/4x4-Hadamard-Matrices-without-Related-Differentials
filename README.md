...
=====================
compiling and running ./a.out i (where i can be {1,2,....,255}), the program generates file containing all 4x4 Hadamard MDS matrices M with M[0,0] = i where the elements satisfy none of the 28 relations shown in the paper. Let us denote the list as L_i. The size of each generated list (named as noRelfile[i].txt) can be approximately 250 MB and each list consists of 14229600 matrices. The repository already has the list of matrices with M[0,0]=1, i.e., L_1 which can be found in the folder named Files (zipped for size restrictions). All the other files can be generated by running ./a.out i (where i = {2,3,..,255).
